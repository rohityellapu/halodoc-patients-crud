...
function EditPatient({ event, reload }) {
    ...
    const onSubmit = async data => {
        ...
        try {
            setisLoading(true)
            const response = await axios.put(apiURL + '/' + event._id,
                { ...data }).then((items) => console.log(items))
                .catch(e => console.log(e));
            setisLoading(false)
            reset();
            setsuccess('Patient is succesfully Updated.')
            reload()
            setTimeout(() => {
    ...
    return (
        <div><Popup
            className=''
            trigger={ <button className="button p-2 bg-blue-500 rounded-md">Edit Patient </button> }
            modal
            nested
        >
            { close => (
                <div className="modal w-96 flex flex-col gap-8 p-4 items-center justify-center bg-black text-white rounded-xl">
                    ...
                    <div className="header text-3xl p-4"> Edit Patient Details </div>
                    <form onSubmit={ handleSubmit(onSubmit) } className='flex flex-col gap-2 text-white rounded-xl bg-black p-4 w-full'>

                        <div className="name">

                            <input defaultValue={ event.name } autoComplete='off' className='bg-black w-full p-2 rounded-md border-none focus:outline-green-400' type="text" placeholder='Patient Name' { ...register('name', { required: "Please enter Patient name." }) } id="" />
                            <p className='text-red-400 text-xs pl-1'>{ errors.name?.message }</p>
                        </div>

                        <div className="contact">

                            <input defaultValue={ event.contact } autoComplete='off' className='bg-black w-full p-2 rounded-md border-none focus:outline-green-400' type="number" placeholder='Contact Number'
                        ...
                        <div className="address">
                            ...
                            <input defaultValue={ event.address } autoComplete='off' className='bg-black w-full p-2 rounded-md border-none focus:outline-green-400' type="text" placeholder='Full Address' { ...register('address', { required: "Please enter Patient address." }) } id="" />
                            <p className='text-red-400 text-xs pl-1'>{ errors.address?.message }</p>
                        </div>
                        <div className="pincode">

                            <input defaultValue={ event.pincode } autoComplete='off' className='bg-black w-full p-2 rounded-md border-none focus:outline-green-400' type="number" placeholder='PIN '
                        ...
                        <div className="actions flex justify-around">
                            <input disabled={ isLoading } type="submit" value="Update" className='p-2 rounded-md bg-blue-600 disabled:opacity-50 hover:cursor-pointer hover:saturate-200' />
                            <button
